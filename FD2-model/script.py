...
# stability memory
SPEAKING_HOLD_FRAMES = 15
BLUR_DELAY_FRAMES = 10
...
track_boxes = {}
prev_mouth_center = {}
mouth_open_history = defaultdict(lambda: deque(maxlen=10))
cached_faces = []
...
while True:
    ...
    curr_time = time.time()
    instant_fps = 1 / (curr_time - prev_time + 1e-6)
    prev_time = curr_time
    fps_display = 0.9 * fps_display + 0.1 * instant_fps
# YOLO
    ...
    for x1, y1, x2, y2, tid in smoothed:
        ...
        if best_face is not None:
            ...
            if kps is not None:
                left = kps[3]
                right = kps[4]
                mouth_center = (left + right) / 2

                if tid in prev_mouth_center:
                    prev = prev_mouth_center[tid]

                    vertical = abs(mouth_center[1] - prev[1])
                    face_h = (y2 - y1)

                    if face_h > 0:
                        vertical /= face_h

                    if vertical < NOISE_THRESHOLD:
                        vertical = 0

                    speaking_frame = vertical > MOUTH_THRESHOLD
                else:
                    speaking_frame = False

                prev_mouth_center[tid] = mouth_center
                mouth_open_history[tid].append(speaking_frame)

                sustained = sum(mouth_open_history[tid]) >= SPEAK_FRAMES

                # distance filter
                face_area = (x2 - x1) * (y2 - y1)
                frame_area = frame.shape[0] * frame.shape[1]
                close = (face_area / frame_area) > MIN_FACE_RATIO

                # speaking memory
                if sustained and close:
                    speaking_memory[tid] = SPEAKING_HOLD_FRAMES
                else:
                    if tid in speaking_memory:
                        speaking_memory[tid] -= 1

                if speaking_memory.get(tid, 0) > 0:
                    is_speaking = True
        ...
        if identity != "pauline":
            if is_speaking:
                blur_delay_memory[tid] = BLUR_DELAY_FRAMES
                should_blur = False
            else:
                if tid in blur_delay_memory:
                    blur_delay_memory[tid] -= 1

                should_blur = blur_delay_memory.get(tid, 0) <= 0
